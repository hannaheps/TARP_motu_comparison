...
#neutral = omnivores
#                    MDS1        MDS2
#Herb_density   -0.1832428 -0.90315614
#InvMob_density -0.2817821 -0.14220369
#Omnivr_density -0.8960418 -0.15006526
#Plktiv_density -0.6160858  0.17069615
#FisCep_density -0.2829735 -0.06519709
#InvSes_density -0.1673878  0.25842331
...
fish_dens_algaen15_ad_notlog
#                           Df SumOfSqs      R2      F Pr(>F)
#N.15_at_30m                 1  0.05392 0.02866 0.6326  0.679
#wind.exposure               1  0.04253 0.02261 0.4989  0.756
#structural_complexity_0to5  1  0.10263 0.05455 1.2040  0.307
#N.15_at_30m:wind.exposure   1  0.06259 0.03327 0.7343  0.590
...
##try with centerered, scaled----
fish_dens_algaen15_ad_log<-adonis2(log(fish.dens.seabirds.trans_sp+1) ~ base::scale(N.15_at_30m) *wind.exposure + base::scale(structural_complexity_0to5), data = fish_fg_dens_dat_levels_algae)
fish_dens_algaen15_ad_log
#same results as aove...
...
#neutral = omnivores
#                      MDS1        MDS2
#Herb_density    0.31773686 -0.32644198
#InvMob_density  0.02143994  0.07044552
#Omnivr_density -0.18921272  0.21096131
#Plktiv_density -0.71437873 -0.17420661
#FisCep_density  0.43917487  0.56232494
#InvSes_density -0.48748034  0.83253420
...
vif(fish_bio_full_mod_algae_log_int) #high vif. 
...
summary(fish_bio_full_mod_algae_log_int3) #no longer singular, and now nothing sig. 
...
#compare to LRTs----
anova(fish_bio_full_mod_algae_log_int2,fish_bio_full_mod_algae_log_int, fish_bio_full_mod_algae_log_int3)
#no different, so can drop structure interactions, but can't drop structure all together
#BUT likely weird because of singularity.....
...
Anova(fish_bio_mod_algae_log_add) #now nothing is significant....
vif(fish_bio_mod_algae_log_add)
...
##try wtih center/scaled:
fish_bio_full_mod_algae_log_int_cs <-lmer(log(sum_fish_biomass)~base::scale(N.15_at_30m) *wind.exposure + base::scale(structural_complexity_0to5)  + (1|site.name), data = fish_comb_dat_levels_algae) 
summary(fish_bio_full_mod_algae_log_int_cs)
Anova(fish_bio_full_mod_algae_log_int_cs)
vif(fish_bio_full_mod_algae_log_int_cs)
#same p-values, etc. as above, but slightly lower vif's....so maybe model is okay. 
...
test(emtrends(fish_bio_full_mod_algae_log_int, ~ 1, var = "structural_complexity_0to5", adjust = "fdr", lmer.df = "asymptotic"))
# overall                             0.91 0.325 Inf   2.801  0.0051
exp(0.91)
...
##Plot fish biomass, in situ according to N15 & exposure
fish_bio_plot<-
fish_comb_dat_levels_algae%>%
  mutate(wind.exposure = relevel(wind.exposure, "windward", "leeward"))%>%
...
fish_comb_dat_levels_algae%>%
  mutate(wind.exposure = relevel(wind.exposure, "windward", "leeward"))%>%
ggplot(aes(x = N.15_at_30m, y = sum_fish_biomass, color = wind.exposure, 
                      fill = wind.exposure, group = wind.exposure))+
  #geom_point(stat = "summary", fun = "mean", alpha = 0.5, size  = 3) +
  geom_point(aes(shape = wind.exposure))+
  geom_line(data = fish_bio_plot_dat.protected, aes(x = N.15_at_30m, y = exp(yvar)))+
   geom_ribbon(data = fish_bio_plot_dat.protected, aes(x = N.15_at_30m, y = exp(yvar), ymin = exp(LCL), ymax=exp(UCL), fill = wind.exposure, color = wind.exposure), alpha = .2, lwd=0)+
    geom_line(data = fish_bio_plot_dat.exposed, aes(x = N.15_at_30m, y = exp(yvar)))+
   geom_ribbon(data = fish_bio_plot_dat.exposed, aes(x = N.15_at_30m, y = exp(yvar), ymin = exp(LCL), ymax=exp(UCL), fill = wind.exposure, color = wind.exposure), alpha = .2, lwd=0)+
...
fish_rich_full_mod_algae_int_glmer2 <-glmer(SR~N.15_at_30m  * wind.exposure + N.15_at_30m  *structural_complexity_0to5  + (1|site.name), family = "poisson", data = fish_comb_dat_levels_algae) 
summary(fish_rich_full_mod_algae_int_glmer2)
Anova(fish_rich_full_mod_algae_int_glmer2) #singular, but nothing sig anyway
...
fish_even_full_mod_algae2 <-lmer(evenness~N.15_at_30m  * wind.exposure + N.15_at_30m  * structural_complexity_0to5  + (1|site.name), data = fish_comb_dat_levels_algae) 
summary(fish_even_full_mod_algae2)
Anova(fish_even_full_mod_algae2) #nothing significant anyway....
...
fish_shannon_full_mod_algae2 <-lmer(shannon~N.15_at_30m  * wind.exposure + N.15_at_30m  * structural_complexity_0to5  + (1|site.name), data = fish_comb_dat_levels_algae) 
summary(fish_shannon_full_mod_algae2)
Anova(fish_shannon_full_mod_algae2) #nothing significant anyway....


```


##plots for even, rich, shannon-----
```{r}
#richness, by n15-----
rich_fish_plot_dat_add <- emmip(fish_rich_full_mod_algae_int_glmer,  ~ N.15_at_30m , at = list(N.15_at_30m  =seq(3.43, 7.0, 0.01)), CIs = TRUE, plotit=FALSE, type = "response") #, type = "response" - not necessary because no transofmratoins

fish_rich_plot<-
ggplot(fish_comb_dat_levels_algae, aes(x = N.15_at_30m, y = SR))+
  geom_point(aes(color = wind.exposure, 
                      fill = wind.exposure, shape = wind.exposure))+
  geom_line(data = rich_fish_plot_dat_add, aes(x = N.15_at_30m, y = yvar))+
   geom_ribbon(data = rich_fish_plot_dat_add, aes(x = N.15_at_30m, y = yvar, ymin = LCL, ymax=UCL), alpha = .2, lwd=0)+
...
#evenness----
even_fish_plot_dat_add <- emmip(fish_even_full_mod_algae,  ~ N.15_at_30m , at = list(N.15_at_30m  =seq(3.43, 7.0, 0.01)), CIs = TRUE, plotit=FALSE, type = "response") #, type = "response" - not necessary because no transofmratoins

fish_even_plot<-
ggplot(fish_comb_dat_levels_algae, aes(x = N.15_at_30m, y = evenness))+
  geom_point(aes(color = wind.exposure, 
                      fill = wind.exposure, shape = wind.exposure))+
  geom_line(data = even_fish_plot_dat_add, aes(x = N.15_at_30m, y = yvar))+
   geom_ribbon(data = even_fish_plot_dat_add, aes(x = N.15_at_30m, y = yvar, ymin = LCL, ymax=UCL), alpha = .2, lwd=0)+
...
#shannon----
shannon_fish_plot_dat_add <- emmip(fish_shannon_full_mod_algae,  ~ N.15_at_30m , at = list(N.15_at_30m  =seq(3.43, 7.0, 0.01)), CIs = TRUE, plotit=FALSE, type = "response") #, type = "response" - not necessary because no transofmratoins

fish_shannon_plot<-
ggplot(fish_comb_dat_levels_algae, aes(x = N.15_at_30m, y = shannon))+
  geom_point(aes(color = wind.exposure, 
                      fill = wind.exposure, shape = wind.exposure))+
  geom_line(data = shannon_fish_plot_dat_add, aes(x = N.15_at_30m, y = yvar))+
   geom_ribbon(data = shannon_fish_plot_dat_add, aes(x = N.15_at_30m, y = yvar, ymin = LCL, ymax=UCL), alpha = .2, lwd=0)+
...
fish_supp_plots<-cowplot::plot_grid(fish_rich_plot, fish_even_plot, fish_shannon_plot, 
                                    nrow = 1, labels = "auto", label_fontface = "plain") #ncol = 1
fish_supp_plots

#ggsave(fish_supp_plots, file = "../output/seabird-fish/fish_supp_plots.jpg",  width = 15, height = 5) #width = 5, height = 10
...
fish_benthic_main_plots<-cowplot::plot_grid(hc_vid_plot, benthic_video_nmds_plot_rotate_new_colours, fish_bio_plot, fish_dens_nmds_plot_rotate.log.newcolors,
                                    nrow = 2, labels = "auto", label_fontface = "plain") 
fish_benthic_main_plots


#ggsave(fish_benthic_main_plots, file = "../output/seabird-fish/fish_and_benthic_main_plots.jpg",  width = 14, height = 10) 
