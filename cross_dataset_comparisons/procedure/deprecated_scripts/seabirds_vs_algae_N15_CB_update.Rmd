...
algae <-
  algae%>%
  unite(site.name, c(Motu, Site), sep = "_", remove = FALSE)

  
  #paste(algae$Motu, seabirds$Site, sep = "_") #seabirds$Exposure,

n15.all.seabirds <- 
  left_join( dplyr::select(algae, c("site.name", "Motu", "Exposure", "Site", "Transect", "Distance_to_shore",  "N15")),
            dplyr::select(seabirds, -c("X", "site.name")),
            by = c("Motu", "Exposure"))%>%
...
###getting different resutls with above as with this:
n15.all.seabirds2 <- merge(algae, seabirds[,-1], by = c("Motu", "Exposure"), all = TRUE, no.dups = TRUE) #"site.name"
names(n15.all.seabirds2)[names(n15.all.seabirds2) == 'Exposure.x'] <- 'Exposure'
names(n15.all.seabirds2)[names(n15.all.seabirds2) == 'Motu.x'] <- 'Motu'

n15.all.seabirds2$reststat <- n15.all.seabirds2$Motu

n15.all.seabirds2$reststat[n15.all.seabirds2$reststat == "Aie"] <- "reference"
n15.all.seabirds2$reststat[n15.all.seabirds2$reststat == "Reiono"] <- "recovering"
n15.all.seabirds2$reststat[n15.all.seabirds2$reststat == "Rimatuu"] <- "unrestored"

n15.all.seabirds2 <- n15.all.seabirds2 %>% mutate(reststat = fct_relevel(reststat, "reference", "recovering", "unrestored"))
#write.csv(n15.all.seabirds2, "n15.all.seabirds2.csv")
#write.csv(n15.all.seabirds, "n15.all.seabirds.csv")
```
...
mod.lmer.rest2 <- lmer(N15 ~ reststat*Distance_to_shore + reststat*Exposure +(1|site.name.y), data = n15.all.seabirds2)

mod.lmer.rest <- lme(N15 ~ reststat*Distance_to_shore + reststat*Exposure, random = (~1|site.name), data = n15.all.seabirds)
mod.lmer.rest2 <- lme(N15 ~ reststat*Distance_to_shore + reststat*Exposure, random = (~1|site.name.x), data = n15.all.seabirds2)
...
mod.lmer.rest <- lmer(N15 ~ reststat*Distance_to_shore + reststat*Exposure +(1|site.name), data = n15.all.seabirds)
mod.lmer.rest2 <- lmer(N15 ~ reststat*Distance_to_shore + reststat*Exposure +(1|site.name), data = n15.all.seabirds2)

n15.all.seabirds<-
  n15.all.seabirds%>%
  unite(site.name.2, "reststat", "Exposure", remove = FALSE )

mod.lmer.rest <- lme(N15 ~ reststat*Distance_to_shore + reststat*Exposure, random = (~1|site.name.2), data = n15.all.seabirds)
mod.lmer.rest <- lmer(N15 ~ reststat*Distance_to_shore + reststat*Exposure +(1|site.name.2), data = n15.all.seabirds)


#test statistics
summary(mod.lmer.rest2)
Anova(mod.lmer.rest2)
...
mod.lm2 <- lm(N15 ~ Motu.x *Distance_to_shore + Motu.x*Exposure.x , data = n15.all.seabirds2)

mod.lm <- lm(N15 ~ Motu*Distance_to_shore + Motu*Exposure, data = n15.all.seabirds)
summary(mod.lm)
summary(mod.lm2)
...
n15.all.seabirds2$site.name
n15.all.seabirds$site.name
n15.all.seabirds2$site.name2
...
emmeans(mod.lmer.rest, list(pairwise ~ Motu|Distance_to_shore|Exposure), type = "response", adjust = "fdr",
        lmer.df = "asymptotic")
emmeans(mod.lmer.rest2, list(pairwise ~ Motu.x|Distance_to_shore|Exposure.x), type = "response", adjust = "fdr",
        lmer.df = "asymptotic")


mod.lm2 <- lm(N15 ~ site.name2, data = n15.all.seabirds2)

mod.lm <- lm(N15 ~ site.name2, data = n15.all.seabirds)
...
emmeans(mod.lmer.rest, list(pairwise ~ reststat|Distance_to_shore|Exposure), type = "response", adjust = "fdr", mode = "asymptotic")
#, lmer.df = "asymptotic"
...
#emmeans for model with all 2-way interactions:
```{r}

emmeans(mod.lmer.rest.2way, list(pairwise ~ Distance_to_shore|Exposure|reststat), type = "response", adjust = "fdr")
#do see the declining distance to shore effect around both exposures for recovering and restored, BUT stronger on protected sides
#no effect of distance to shroe on either side for unrestored

emmeans(mod.lmer.rest.2way, list(pairwise ~ Distance_to_shore|Exposure|reststat), type = "response", adjust = "fdr", lmer.df = "asymptotic")
#not a big difference to use asymptotic versus knward-roger, so that's good :)
...
#no 3-way interaction, phew. And no distance to shore*exposure interaction.
...
plot_dat2b<-emmip(mod.lmer.breeding, Distance_to_shore ~ breeding_biomass_kgha_side, at = mylist2, CIs = TRUE, plotit=FALSE)
plot_dat2

n15.all.seabirds %>%
   ggplot(aes(x = breeding_biomass_kgha_side, y = N15, color = Distance_to_shore, fill = Distance_to_shore, group = Distance_to_shore))+
  geom_point(alpha = .5, aes(shape = Exposure))+
   geom_ribbon(data = plot_dat2b, aes(x = breeding_biomass_kgha_side, y = yvar, ymin = LCL, ymax=UCL, fill = Distance_to_shore), alpha = .2, lwd=0)+
 geom_line(data = plot_dat2b, aes(x = breeding_biomass_kgha_side, y = yvar, color = Distance_to_shore, group = Distance_to_shore))+
  theme_bw() 

```


#run with seabird breeding biomass by motu, so then have same range of seabird biomass for both sides:
```{r}
##raw plot:----
n15.all.seabirds %>%
...
##try with 3-way interaction----
mod.lmer.breeding.motu3 <- lmer(N15 ~ breeding_biomass_kgha_motu*Distance_to_shore*Exposure + (1|site.name), n15.all.seabirds)
Anova(mod.lmer.breeding.motu3)


test(emtrends(mod.lmer.breeding.motu3, ~ Distance_to_shore|Exposure, var = "breeding_biomass_kgha_motu",  adjust = "fdr", type = "response"))
#if change to asymptotic df, then significant on protected side. 
#same results as above*

emtrends(mod.lmer.breeding.motu3,  pairwise~ Distance_to_shore|Exposure, var = "breeding_biomass_kgha_motu", adjust = "fdr")
#DIFFERENT than above - now nothing sig on exposed side, 10 m different than all others on protected side**
