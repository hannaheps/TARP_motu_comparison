...
scores(nmds_fish_dens_autoT.rotate, display="species")
cor(fish.dens.seabirds.trans_sp,
      nmds_fish_dens_autoT.rotate$points,
      use = "complete.obs",
      method = "pearson")
...
ord.fit.algae.autoT.rotate<-envfit(nmds_fish_dens_autoT.rotate~N.15_at_30m+protection + structural_complexity_0to5, data = fish_fg_dens_dat_levels_algae, na.rm=TRUE)
ord.fit.algae.autoT.rotate #Algae n15 is significant****
plot(nmds_fish_dens_autoT.rotate)
plot(ord.fit.algae.autoT.rotate)
...
nmds_fish_site_scores_rotate.autoT.w.mnmds<-
 left_join(nmds_fish_site_scores_rotate.autoT,
           fish_fg_dens_dat_levels_algae, 
             by = c("site.name", "transect")) 
nmds_fish_site_scores_rotate.autoT.w.mnmds
...
##get arrows: - UPDATED, NEED TO MULTIPLY BY * ordiArrowMul(ord.fit.fish.rotate.autoT))
sb.scrs.fish.rotate.autoT   <- as.data.frame(scores(ord.fit.algae.autoT.rotate  , display = "vectors")* ordiArrowMul(ord.fit.algae.autoT.rotate))
...
fish_dens_nmds_plot_rotate.autoT.newcolors<-
ggplot() + 
  geom_point(data=nmds_fish_site_scores_rotate.autoT.w.mnmds ,aes(x=NMDS1,y=NMDS2,colour=N.15_at_30m, fill = N.15_at_30m), stat="identity", size=5, alpha = .9) +
...
#coord_fixed() + ## need aspect ratio of 1!
  geom_text(data=fish_spp.sc.rotate.autoT ,aes(x=NMDS1,y=NMDS2 ,label=species2), alpha = .5, hjust = "inward") +  # add the species labels - 
  geom_segment(data = sb.scrs.fish.rotate.autoT ,
               aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
               arrow = arrow(length = unit(0.25, "cm")), colour = "black") +
    geom_text(data = sb.scrs.fish.rotate.autoT, aes(x = NMDS1, y = NMDS2, label = variable2), 
...
#save plot
ggsave(fish_dens_nmds_plot_rotate.autoT.newcolors, file = "../output/seabird-fish/fish_nmds_plot_rotate_new_colours.jpg",
    width = 7, height = 5)
...
vif(fish_dens_algaen15_ad_log)

simper_fish_n15<-simper(fish.dens.seabirds.trans_sp, fish_fg_dens_dat_levels_algae$N.15_at_30m, permutations = 999)
summary(simper_fish_n15)
...
fish_dens_algaen15_ad_wisc<-adonis2(wisconsin(fish.dens.seabirds.trans_sp) ~ N.15_at_30m  + wind.exposure + structural_complexity_0to5, data = fish_fg_dens_dat_levels_algae)
fish_dens_algaen15_ad_wisc
#N.15_at_30m                 1  0.20818 0.11720 2.8461  0.024 *
#wind.exposure               1  0.05222 0.02940 0.7139  0.586  
#structural_complexity_0to5  1  0.05301 0.02984 0.7247  0.631  
#Residual                   20  1.46293 0.82357                
#Total                      23  1.77633 1.00000   


##wisconsin standardization----
fish_dens_motu_ad_wisc<-adonis2(wisconsin(fish.dens.seabirds.trans_sp) ~ motu  + wind.exposure + structural_complexity_0to5, data = fish_fg_dens_dat_levels_algae)
fish_dens_motu_ad_wisc
#                           Df SumOfSqs      R2      F Pr(>F)
#motu                        2  0.15338 0.08635 0.9526  0.522
#wind.exposure               1  0.05557 0.03128 0.6902  0.629
#structural_complexity_0to5  1  0.03778 0.02127 0.4693  0.792
#Residual                   19  1.52961 0.86110              
#Total                      23  1.77633 1.00000  
#no effect for wisconsind or log. (motu = restoration)
...
#run nmds----
#run NMDS (data frame created above)
nmds_fish_dens_autoF<-metaMDS(fish.dens.seabirds.trans_sp, distance = "bray", trymax=200, k=2, autotransform = TRUE)
nmds_fish_dens_autoT
#2 axis stress = .1378
...
###UPDATE - CHECKING NON-LINEARITIES
```{r}
#use log again: 
fish_bio_full_mod_algae_log_poly <-lmer(log(sum_fish_biomass)~N.15_at_30m + poly(N.15_at_30m,2)  + wind.exposure + structural_complexity_0to5  + (1|site.name), data = fish_comb_dat_levels_algae) #
...
#species richness----
fish_rich_full_mod_algae_poly <-lmer(SR~N.15_at_30m + poly(N.15_at_30m,2)   + wind.exposure + structural_complexity_0to5  + (1|site.name), data = fish_comb_dat_levels_algae) #depth_max_m + start_distance_to_shore_m
summary(fish_rich_full_mod_algae_poly)
plot(fish_rich_full_mod_algae)
qqnorm(resid(fish_rich_full_mod_algae))
qqline(resid(fish_rich_full_mod_algae))
#SINGULAR
...
Anova(fish_rich_full_mod_algae_poly) #structure significant****
vif(fish_rich_full_mod_algae) #all fine

anova(fish_rich_full_mod_algae, fish_rich_full_mod_algae_poly)
#poly fit is better - is this because of singular???
#but aic still within 2*** and p = 0.046

#species evenness----
fish_even_full_mod_algae_poly <-lmer(evenness~N.15_at_30m + poly(N.15_at_30m,2)   + wind.exposure + structural_complexity_0to5  + (1|site.name), data = fish_comb_dat_levels_algae) #singular
summary(fish_even_full_mod_algae_poly)
plot(fish_even_full_mod_algae_poly)
qqnorm(resid(fish_even_full_mod_algae_poly))
#looks okay

Anova(fish_even_full_mod_algae_poly) #nothing significant anyway....
plot_summs(fish_even_full_mod_algae_poly)

anova(fish_even_full_mod_algae, fish_even_full_mod_algae_poly)
#no diff

#shannon----
fish_shannon_full_mod_algae_poly <-lmer(shannon~N.15_at_30m + poly(N.15_at_30m,2)   + wind.exposure + structural_complexity_0to5  + (1|site.name), data = fish_comb_dat_levels_algae)   
summary(fish_shannon_full_mod_algae_poly)
plot(fish_shannon_full_mod_algae_poly)
qqnorm(resid(fish_shannon_full_mod_algae_poly))

Anova(fish_shannon_full_mod_algae_poly) #nothing significant****
plot_summs(fish_shannon_full_mod_algae_poly)

anova(fish_shannon_full_mod_algae, fish_shannon_full_mod_algae_poly)
#no difference

#try without other predictors for singular:
fish_rich_full_mod_algae_poly2 <-lmer(SR~N.15_at_30m + poly(N.15_at_30m,2)  + (1|site.name), data = fish_comb_dat_levels_algae) #depth_max_m + start_distance_to_shore_m
summary(fish_rich_full_mod_algae_poly2)

fish_rich_full_mod_algae_linear2 <-lmer(SR~N.15_at_30m + (1|site.name), data = fish_comb_dat_levels_algae) #depth_max_m + start_distance_to_shore_m
summary(fish_rich_full_mod_algae_linear2)
Anova(fish_rich_full_mod_algae_linear2)

anova(fish_rich_full_mod_algae_poly2, fish_rich_full_mod_algae_linear2)
#now no difference**
...
fish_comb_dat_levels_algae%>%
  select(motu, site.name, wind.exposure, transect, structural_complexity_0to5, N.15_at_30m, FisCep_density,
...
fish_comb_dat_levels_algae%>%
  select(motu, site.name, wind.exposure, transect, structural_complexity_0to5, N.15_at_30m, FisCep_biomass,
