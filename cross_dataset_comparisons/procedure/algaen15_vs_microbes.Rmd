...
mod.glmm1 <- glmer(Observed~algae.N15*Distance_to_shore + Exposure + (1|site.name), family = "poisson", data = coral.microbes.seabirds)
...
#Can't do a post-hoc test bc we do not have levels. 
...
ggsave("../output/seabird-microbes/organised_output/1_mod_coral_richness.jpg", plot = p.glmm1, width = 7, height = 5)
...
ggsave("../output/seabird-microbes/organised_output/2_mod_coral_richness_dist.jpg", plot = p.glmm1.dist, width = 7, height = 5)
...
coral_rich_algae_lm_quad1<-lmer(Observed ~ algae.N15*Distance_to_shore + Exposure + (1|site.name) + I(algae.N15^2)*Distance_to_shore, 
                          data = coral.microbes.seabirds.naomit)


coral_rich_algae_lm_quad2<-lmer(Observed ~ algae.N15*Distance_to_shore + Exposure + (1|site.name) + poly(algae.N15,2)*Distance_to_shore, 
                          data = coral.microbes.seabirds.naomit)
...
test(emtrends(coral_rich_algae_lm_quad1, ~ Distance_to_shore, var = "algae.N15", adjust = "fdr", max.degree=2))
...
coral_rich_algae_lm_poly <- lmer(Observed ~ poly(algae.N15, degree = 2)*Distance_to_shore + Exposure + (1|site.name), data = coral.microbes.seabirds.naomit)

test(emtrends(coral_richness_algae_lm_poly, ~ Distance_to_shore, var = "algae.N15", adjust = "fdr", max.degree=2))
...
plot_dat3<-emmip(coral_rich_algae_lm_poly, Distance_to_shore ~ algae.N15, at = mylist3, CIs = TRUE, plotit=FALSE)
plot_dat3


p.poly.cor.rich <- coral.microbes.seabirds.naomit %>%
      mutate(Distance_to_shore = fct_relevel(Distance_to_shore, c("10", "20", "30", "40")))%>%
   ggplot(aes(x = algae.N15, y = Observed, color = Distance_to_shore, fill = Distance_to_shore, group = Distance_to_shore))+
  geom_point(alpha = .5)+
   geom_ribbon(data = plot_dat3, aes(x = algae.N15, y = yvar, ymin = LCL, ymax=UCL, fill = Distance_to_shore), alpha = .2, lwd=0)+
 geom_line(data = plot_dat3, aes(x = algae.N15, y = yvar, color = Distance_to_shore, group = Distance_to_shore))+
  theme_bw() 
...
coral.rich.n15.quad2<-lmer(Observed ~ algae.N15 + Exposure + (1|site.name) + poly(algae.N15,2), 
                          data = coral.microbes.seabirds.naomit)
...
plot_dat3<-emmip(coral.rich.n15.poly,  ~ algae.N15, at = mylist3, CIs = TRUE, plotit=FALSE)
plot_dat3
...
#Not much of a difference from linear tbh??
ggsave("../output/seabird-microbes/organised_output/1c_mod_poly_cor_richness_n15only.jpg", plot = p.poly.cor.rich.n15, width = 7, height = 5)
...
p.alter <- effect_plot(mod.altero, pred = algae.N15, interval = TRUE, plot.points = TRUE, jitter = 0.2, cat.geom= line, data = beta_trans_coral_microbes)
