...
algal_even_plot
#not sure why error bars are a little wiggly for windward....reduce seq step then it looks better:----
...
test(emtrends(shannon.lmer, ~1, var = "mean_n15_30m", adjust = "fdr"))
# overall             -0.217 0.0464 44  -4.672  <.0001
#this has no transformations, so can interpret as is, i.e.:
...
#ggsave("../output/seabird-algaediv/algal_shannon_plot_CB.jpg", plot = algal_shannon_plot, width = 7, height = 5)
...
algal_cover_plot<-
ggplot(algae.div.seabirds.beta, aes(x = mean_n15_30m, y = algal.cover))+
  geom_point(aes(color = exposure, 
                      fill = exposure, shape = exposure))+
...
#ggsave("../output/seabird-algaediv/algal_cover_plotCB.jpg", plot = algal_cover_plot, width = 7, height = 5)
...
#probably could do glmer with beta (because proportion data)----
turb.glmer<-glmmTMB(Turbinaria.ornata~mean_n15_30m*exposure + (1|site.name), family=beta_family(), data = algae.div.seabirds.beta)
summary(turb.glmer)
plot(resid(turb.glmer))
qqnorm(resid(turb.glmer))
hist(resid(turb.glmer))

#looks good

Anova(turb.glmer)
#mean_n15_30m          1.1361  1   0.286469   
#exposure              0.8744  1   0.349753   
#mean_n15_30m:exposure 7.0847  1   0.007774 **

#results similar to lmer -  but now even more sig interaction


##emmeans---------
test(emtrends(turb.glmer, ~ exposure, var = "mean_n15_30m", adjust = "fdr"))
# exposure  mean_n15_30m.trend     SE    df t.ratio p.value
...
test(emtrends(turb.glmer, ~ exposure, var = "mean_n15_30m", adjust = "fdr", lmer.df = "asymptotic"))
# Exposed                 1.02 0.854 Inf   1.194  0.2325
# Protected              -1.01 0.414 Inf  -2.439  0.0295

#smaller p-values, but results/conclusions similar

test(emtrends(turb.glmer, pairwise~ exposure, var = "mean_n15_30m", adjust = "fdr"))
# contrast            estimate   SE  df z.ratio p.value
...
####plots----
turb_plot_dat <- emmip(turb.glmer,  ~ mean_n15_30m|exposure, at = list(mean_n15_30m = seq(3.43, 9.58, .01)), CIs = TRUE, plotit=FALSE, type = "response") #, type = "response" - not necessary because no transofmratoins
...
ggplot(algae.div.seabirds.beta, aes(x = mean_n15_30m, y = Turbinaria.ornata, color = exposure, 
                      fill = exposure, group = exposure))+
  #geom_point(stat = "summary", fun = "mean", alpha = 0.5, size  = 3) +
  geom_point(aes(shape = exposure))+
...
algae_main_plots<-cowplot::plot_grid(algae_rich_glmer_CB, algal_even_plot, algal_cover_plot, algae.nmds.plot.rotate.exposure, labels = "auto", label_fontface = "plain")
algae_main_plots
