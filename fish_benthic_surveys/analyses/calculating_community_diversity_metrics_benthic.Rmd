...
plot(ord.fit)
#draw polygons
ordihull(nmds_video_log, tet_video_raw_meta_broad$site.name, pty=3)
...
tet_video_div_comm_dat_w_cover<-
  tet_video_div_comm_dat_w_cover%>%
  mutate(across(c(total_hard_coral:transect_tape), round, 2))%>%
  rename_with(.fn = ~ paste0(.x, "_percent_cover"), .cols = c("total_hard_coral":"total"))
tet_video_div_comm_dat_w_cover
...
#clam + sea cucmbur to other invert
tet_insitu_raw_meta<-
  tet_insitu_raw_meta%>%
...
#pavement + turf (because some observer differences - I called more turf, but only did Rimatuu Exposed and Reiono Protected. Rosie called pavement)
tet_insitu_raw_meta<-
  tet_insitu_raw_meta%>%
...
```{r}
#run nmds----
nmds_insitu_log<-metaMDS(tet_insitu_raw_meta_broad_sp, distance = "bray", trymax=200, k=2, autotransform = FALSE)
nmds_insitu_log
#2 axes stress = 0.06666792, so all good

plot(nmds_insitu_log)
scores(nmds_insitu_log, display="species")


#look at plot and centroids, for now just motu and exposure (so sitename). will need to add explanatory variables
plot(nmds_insitu_log)
ord.fit<-envfit(nmds_insitu_log~site.name, data = tet_insitu_raw_meta_cat_wide, na.rm=TRUE)
ord.fit #looks like some differences among motu
plot(ord.fit)
#draw polygons
ordihull(nmds_insitu_log, tet_insitu_raw_meta_cat_wide$site.name, pty=3)
...
##extract site/transect scores----
insitu_spp.sc <- as.data.frame(scores(nmds_insitu_log, "species"))  #Using the scores function from vegan to extract the species scores and convert to a data.frame
insitu_spp.sc<-
  insitu_spp.sc%>%
  rownames_to_column(var="Species") # create a column of species, from the rownames of species.scores


insitu_site.sc <- scores(nmds_insitu_log, display = "sites", shrink = FALSE) 
insitu_site.sc
...
#hard coral
cor.test(tet_insitu_div_comm_dat_w_cover$coral_prop_cover, tet_video_div_comm_dat_w_cover$total_hard_coral_percent_cover)
#0.7463035 
...
#0.4166722
#but, no turf category in in situ:
cor.test(tet_insitu_div_comm_dat_w_cover$pavement_prop_cover, (tet_video_div_comm_dat_w_cover$pavement_percent_cover+ tet_video_div_comm_dat_w_cover$turf_percent_cover))
#0.3118573 - that's actually worse....
...
#save(tet_video_div_comm_dat_w_cover, tet_insitu_div_comm_dat_w_cover, nmds_fish_diet_log_site_scores, file = "../output/data/benthic_community_data.Rdata")
