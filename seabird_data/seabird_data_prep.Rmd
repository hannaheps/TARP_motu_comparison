...
sb_dat_focal_nests_motu_sp<-
  ...
  summarize(BreedingPairs = sum(Nb),
            Abundance = BreedingPairs)%>% #- originally multipled by 2 assuming 2 adults per nest (so # of seabirds rather than # nests or breeding pairs) - CHANGED TO 1 based on Jayna's email that only usually 1 bird on a nest at a time pooping***
  mutate(Density_m2 = Abundance/(500*Number_transects)) %>% #5 x 100 m transects, so each transect = 500 m, times the number of transects
  mutate(Density_ha = Density_m2*10000) %>% #and convert to hectares because that's used a lot
 mutate(biomass_kg = Abundance*Body_mass_HBW/1000, #calculate biomass per species  - divide by 1000 to go from grams to kg
         biomass_kg_m2 = biomass_kg/(500*Number_transects), #convert to per m^2 as above
...
sb_dat_focal_nests_100m_sp<-
 ...
 mutate(biomass_kg = Abundance*Body_mass_HBW/1000, #calculate biomass per species - divide by 1000 to go from grams to kg
         biomass_kg_ha = biomass_kg/500*10000) #convert to per hectare - divide by 500 because 500 m&2 per transect, multiple by 10000 for m^2 to hectare
     #  mutate(biomass_kg_ha2 = Density_ha*(Body_mass_HBW/1000)) #THIS IS EQUIVALENT TO BIOMASS_KG_HA CALCULATION
...
sb_dat_focal_nests_200m_sp<-
  ...
  mutate(Density_ha = Density_m2*10000)%>% #and convert to hectares because that's used a lot
 mutate(biomass_kg = Abundance*Body_mass_HBW/1000, #calculate biomass per species - divide by 1000 to go from grams to kg
         biomass_kg_ha = biomass_kg/(500*2)*10000) #convert to per hectare - divide by 500*2 because 500 m^2 per transect and 2 transects, multiple by 10000 for m^2 to hectare
         #  mutate(biomass_kg_ha2 = Density_ha*(Body_mass_HBW/1000)) #THIS IS EQUIVALENT TO BIOMASS_KG_HA CALCULATION
...
seabird_focal_all%>%
  mutate(Motu_exposure = str_c(Motu, Exposure, sep ="_"))%>%
  ggplot(aes(x = breeding_biomass_kgha_100m, y = breeding_biomass_kgha_side, fill = Motu_exposure, color = Motu_exposure))+
  geom_point(size = 5, alpha= .6)+
  theme_bw()

seabird_focal_all%>%
  mutate(Motu_exposure = str_c(Motu, Exposure, sep ="_"))%>%
  ggplot(aes(x = breeding_biomass_kgha_200m, y = breeding_biomass_kgha_side, fill = Motu_exposure, color = Motu_exposure))+
  geom_point(size = 5, alpha= .6)+
  theme_bw()

seabird_focal_all%>%
  mutate(Motu_exposure = str_c(Motu, Exposure, sep ="_"))%>%
  ggplot(aes(x = breeding_biomass_kgha_motu, y = breeding_biomass_kgha_side, fill = Motu_exposure, color = Motu_exposure))+
  geom_point(size = 5, alpha= .6)+
  theme_bw()
...
#conduct actual correlation test----
cor(seabird_focal_all_r[,3:18])

#try wtih rstatix package:
library(rstatix)
cor_test(seabird_focal_all_r[,3:18]) 

#pick out most important comparisons:
#compare breeding biomass across all spatial scales:
br_bio_cor<-cor_test(seabird_focal_all_bio[,3:6]) 
br_bio_cor


#compare adult biomass across all spatial scales:
adult_bio_cor<-cor_test(seabird_focal_all_bio[,7:10]) 
adult_bio_cor

#compare breeding versus adult biomass, density vs. biomass within each spatial scale:
#100 m:
seabird_focal_all_r_100<-
seabird_focal_all_r%>%
    select(contains("100m"))
seabird_focal_all_r_100

cor_100<-cor_test(seabird_focal_all_r_100[,2:5]) 
  cor_100

#200 m:
seabird_focal_all_r_200<-
seabird_focal_all_r%>%
    select(contains("200m"))
seabird_focal_all_r_200

cor_200<-cor_test(seabird_focal_all_r_200[,2:5]) 
  cor_200
  
#side:
seabird_focal_all_r_side<-
seabird_focal_all_r%>%
    select(contains("side"))
seabird_focal_all_r_side

cor_side<-cor_test(seabird_focal_all_r_side[,2:5]) 
  cor_side
  
  
#motu:
seabird_focal_all_r_motu<-
  seabird_focal_all_r%>%
    select(contains("motu"))
seabird_focal_all_r_motu

cor_motu<-cor_test(seabird_focal_all_r_motu[,2:5]) 
  cor_motu
  
  

#save cor test outputs----
#write.csv(cor_motu, "outputs/seabird_corr_by_motu.csv")
#write.csv(cor_side, "outputs/seabird_corr_by_side.csv")
#write.csv(cor_200, "outputs/seabird_corr_by_200m.csv")
#write.csv(cor_100, "outputs/seabird_corr_by_100m.csv")
#write.csv(br_bio_cor, "outputs/seabird_corr_breeding_biomass.csv")
#write.csv(adult_bio_cor, "outputs/seabird_corr_adult_biomass.csv")
...
sb_dat_iti_nests_sp<-
  ...
  group_by(Motu, Number_transects, SpeciesName, Body_mass_HBW)%>%
  summarize(BreedingPairs = sum(Nb),
            Abundance = BreedingPairs)%>% #Original breeding density/biomass estimates are assuming 2 birds per nest, as per one of my previous papers. But, Jayna suggested only 1 bird per nest because typically only one there pooping at a time. So changed.
  mutate(Density_m2 = Abundance/(500*Number_transects)) %>% #5 x 100 m transects, so each transect = 500 m, times the number of transects
  mutate(Density_ha = Density_m2*10000) %>% #and convert to hectares because that's used a lot
 mutate(biomass_kg = Abundance*Body_mass_HBW/1000, #calculate biomass per species  - divide by 1000 to go from grams to kg
         biomass_kg_m2 = biomass_kg/(500*Number_transects), #convert to per m^2 as above
...
#repeat corr tests, with Iti data:
```{r}
cor_test(seabird_side_w_iti[,3:6]) 

#pick out most important comparisons:
#compare all within spatial scales (side for iti):
cor_iti<-cor_test(seabird_side_w_iti[,3:6]) 
cor_iti
#still highly correlated

#combine iti data with full data, then compare within spatial scales:
seabird_focal_all
seabird_side_w_iti

seabird_side_w_iti_c<-
  seabird_side_w_iti%>%
  rename_with(~paste0("iti_", .x, recycle0 = TRUE), contains("side"))


seabird_focal_all_and_iti<-
  full_join(seabird_focal_all, seabird_side_w_iti_c, by = c("Motu", "Exposure"))
seabird_focal_all_and_iti

#more corr tests:

#side:
seabird_focal_all_and_iti_side<-
seabird_focal_all_and_iti%>%
    select(contains("side"))
seabird_focal_all_and_iti_side

cor_iti_side<-cor_test(seabird_focal_all_and_iti_side[,2:9]) 
  cor_iti_side
#correlation goes away***

#compare breeding biomass across scales:
seabird_focal_all_and_iti_br_bio<-
  seabird_focal_all_and_iti%>%
    select(contains("breeding_biomass"))
seabird_focal_all_and_iti_br_bio
  
cor_br_bio_w_iti<-cor_test(seabird_focal_all_and_iti_br_bio[,2:6]) 
  cor_br_bio_w_iti
  
#yup, correlations with iti side are really bad**

#save additional cor test outputs----
#write.csv(cor_br_bio_w_iti, "outputs/seabird_corr_br_bio_w_iti.csv")
#write.csv(cor_iti_side, "outputs/seabird_corr_by_side_w_iti.csv")
#write.csv(cor_iti, "outputs/seabird_corr_side_iti_only.csv")
...
#     sb_dat_iti_adults_sp, sb_dat_iti_nests_sp,
#     seabird_focal_all, seabird_side_w_iti, file = "outputs/seabird_dens_bio_data.RData")
